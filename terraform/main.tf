...
# Project: Date My Family (DStv · Mzansi Magic · Channel 161)
# ------------------------------------------------------------------------------
...
resource "aws_kms_key" "media_key" {
  description             = "KMS Key for Date My Family media asset encryption"
  deletion_window_in_days = 30
  enable_key_rotation     = true

  tags = {
    Name = "date_my_family_production_workflow-kms-key"
  }
}

# 2. S3 Media Vault Bucket
resource "aws_s3_bucket" "media_vault" {
  bucket        = "nwhite-date-my-family-production-workflow-${var.environment}"
  force_destroy = false
}
...
resource "aws_iam_role" "post_production_role" {
  name = "date_my_family_production_workflow-post-production-role"
...
resource "aws_iam_policy" "post_production_policy" {
  name        = "date_my_family_production_workflow-post-production-policy"
  description = "Scoped S3 and KMS permissions for Date My Family post-production workflow"
...
resource "aws_cloudwatch_log_group" "workflow_logs" {
  name              = "/aws/media-pipeline/date_my_family_production_workflow"
